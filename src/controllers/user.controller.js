  ...
  const user = await User.aggregate([
    {
      //match the user id
      $match: {
        //match the user id we cannot use direct req.user._id because we are using aggregate because of that mongoose did not automatically convert the req.user._id to object id
        ...
        pipeline: [
          {
            $lookup: {
              ...
              pipeline: [
                {
                  //project the owner details which we need
                  $project: {
                    fullName: 1,
                    username: 1,
                    avatar: 1,
